...
/**
 *  set up scrollView's content data source， the contentViews
 */
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if(contentOffsetX >= (2 * CGRectGetWidth(scrollView.frame))) {
        ...
        //        NSLog(@"next，current page:%d",self.currentPageIndex);
        _pageControl.currentPage = self.currentPageIndex;
        [self configContentViews];
    }
    if(contentOffsetX <= 0) {
        self.currentPageIndex = [self getValidNextPageIndexWithPageIndex:self.currentPageIndex - 1];
        //        NSLog(@"previous，current page:%d",self.currentPageIndex);
        _pageControl.currentPage = self.currentPageIndex;
        [self configContentViews];
    }
}
